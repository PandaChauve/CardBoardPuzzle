    ...
    export class Drawer{
        ...
        private getGridEventHandler() : (event: Utils.IEvent) => void{
            ...
            return function(event : Utils.IEvent):void{
                if(event.name == 'newToken'){
                    var cube = new PandaCardBoard.GameToken(event.content.x*1.5 - that._grid.getSize()*0.75, 0, event.content.y*1.5-that._grid.getSize()*0.75, event.content.color-1 );
        ...
        private drawGrid() : void{
            ...
            for(let idx = 0; idx < this._grid.getSize(); ++idx){
                var cube = new PandaCardBoard.Button(idx*1.5 - this._grid.getSize()*0.75, 0, this._grid.getSize()*1.5+2 - this._grid.getSize()*0.75, idx);
